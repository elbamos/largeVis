...
inline void Gradient::multModify(coordinatetype *col, coordinatetype adj) const {
...
ExpGradient::ExpGradient(const distancetype g, const dimidxtype d) : Gradient(g, d), gammagamma(gamma * gamma) {
		cap = gamma;
};
