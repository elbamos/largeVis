# This file was generated by Rcpp::compileAttributes
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

sgd <- function(coords, is, js, ps, ws, gamma, rho, minRho, useWeights, nBatches, M, alpha, callback) {
    invisible(.Call('largeVis_sgd', PACKAGE = 'largeVis', coords, is, js, ps, ws, gamma, rho, minRho, useWeights, nBatches, M, alpha, callback))
}

searchTrees <- function(threshold, n_trees, K, max_recursion_degree, maxIter, data, callback) {
    .Call('largeVis_searchTrees', PACKAGE = 'largeVis', threshold, n_trees, K, max_recursion_degree, maxIter, data, callback)
}

distance <- function(is, js, xs, data, callback) {
    invisible(.Call('largeVis_distance', PACKAGE = 'largeVis', is, js, xs, data, callback))
}

distMatrixTowij <- function(is, js, xs, sigmas, N, callback) {
    .Call('largeVis_distMatrixTowij', PACKAGE = 'largeVis', is, js, xs, sigmas, N, callback)
}

sigFunc <- function(sigma, x_i, perplexity) {
    .Call('largeVis_sigFunc', PACKAGE = 'largeVis', sigma, x_i, perplexity)
}

