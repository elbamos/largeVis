...
gplot <- function(x, coords, text = FALSE) {
  ...
  plt <- ggplot2::ggplot(dframe, 
    ...
    ggplot2::geom_point(aes(alpha = probabilities), size = 0.7) + 
    ggplot2::geom_segment(size = 0.5, ggplot2::aes(alpha = lambda, size = lambda))
  if (text == "parent") {
    plt <- plt + ggplot2::geom_label(ggplot2::aes(label = parent), size = 2.5, 
                            label.padding = ggplot2::unit(0.1, "lines"), 
                            label.size = 0.1, alpha = 0.7)
  } else if (text) {
    plt <- plt + ggplot2::geom_label(ggplot2::aes(label = label), size = 2.5, 
                            label.padding = ggplot2::unit(0.1, "lines"), 
                            label.size = 0.1, alpha = 0.7)
  }
  plt
