...
if (rebuild) {
  ...
  darch::provideMNIST(download=T, folder = "./mnist/")
  load("mnist/train.RData")
  load("mnist/test.RData")
...
```{r mnistmanifold,echo=F,eval=F,results='asis',fig.width=8,fig.height=8,message=F,warning=F,fig.align='center'}
if (!exists("mnistData")) {
  load("./mnist/train.RData")
  load("./mnist/test.RData")
  mnistData <- rbind(trainData,testData)
...
```{r drawwikiwords,echo=F,eval=F,warning=F,message=F,results='asis'}
...
if (rebuild) {
  ...
  wikidocs <- readr::read_delim("./Wiki_embedding/doc_vec",delim= " ", 
                                 col_names = FALSE, 
                                 skip = 1)
  docdata <- t(as.matrix(wikidocs[, 2:101]))
  set.seed(1974)
  initcoords <- matrix(rnorm(ncol(docdata) * 2), nrow = 2)
  wikiVis <- vis(docdata, 
                 coords = initcoords, 
                 K = 100, 
                 tree_threshold = 100, 
...
```
```{r drawwikidocs,echo=F,eval=F,warning=F,message=F,results='asis'}
datapath <- paste(vdatapath, "wikidocs.Rda", sep = "")
if (file.exists(datapath)) {
  load(datapath) 
...
if (rebuild) {
  ...
  library(Rtsne)
  tsneCoords <- data.frame(scale(Rtsne(t(theta), pca = FALSE)$Y))
