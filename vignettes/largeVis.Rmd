...
See the [original paper](https://arxiv.org/abs/1602. ggplot(mnistCoords, aes(x = x, y = y, color = labels)) +
    geom_point(size = 0.1, alpha = 0.3) +
    scale_x_continuous(name = "", limits = c(-45, 45), breaks = NULL) +
    scale_y_continuous(name = "", limits = c(-45, 60), breaks = NULL)00370) for a detailed description of the algorithm. 
...
data(wiki)
inputs <- data.frame(
  g = rep(7, 3) ,# rep(c(.5,1,7,14), 5),
  a = c(0,1, 5) #rep(c(0,.1,1,5,10), each = 4)
)
set.seed(1974)
