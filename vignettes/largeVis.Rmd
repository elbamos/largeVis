...
The following generates a grid illustrating the effect of the $\alpha$ and $\gamma$ hyperparameters on the `mnist` dataset:
...
```{r drawhyperparameters,echo=T,eval=F}
ggplot(agcoords,
       aes(x = x, 
           y = y, 
           color = factor(label))) +
...
The following code generates a chart illustrating the effect of the `M` hyperparameter from `projectKNNs` and `K`, the number of nearest neighbors, again using the `mnist` dataset.
