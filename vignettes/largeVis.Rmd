...
Note that the input data structure must be transposed before being passed to `randomProjectionTreeSearch`. (The reason is to reduce memory requirements when working with very large datasets.) For example:

```{r iris}
data(iris)
dat <- t(as.matrix(iris[, 1:4]))
neighbors <- randomProjectionTreeSearch(dat, K = 30, n_trees = 10, distance_method = "Euclidean")
```

The returned object has two components. `neighbors` is a 0-indexed, sorted, table of the K nearest neighbors for eachp oint. `edgematrix` is a sparse (non-symmetric) matrix of the distances from each point to its K nearest neighbors:
...
In basic usage, `projectKNNs` takes an $\omega$ matrix (see the original paper for details) as its primary parameter, and returns a transposed matrix of coordinates for the input points:

```{r projectKnns}
omega <- buildWijMatrix(neighbors$edgematrix)
coords <- projectKNNs(omega, dim = 2, seed = 1974)
par(mai = c(0.1, 0.1, 0.1, 0.1), mgp = c(0, 0, 0))
plot(t(coords), col = iris$Species, xlab = "", ylab = "", pch = "x", cex = 0.5, xaxt = "n", yaxt = "n")
```
...
if (.build_mnist) {
	...
	agcoords <- dplyr::bind_rows(purrr::map2(inputs$alpha, inputs$gamma, function(a, g) {
		obj <- projectKNNs(visObj$wij, dim = 2, alpha = a, gamma = g, seed = 1974)
		obj <- data.frame(scale(t(obj))) 
		obj <- set_colnames(obj, c("x", "y"))
		mutate(obj, alpha = a, gamma = g)
	}))
...
if (.build_mnist) {
	...
	kmcoords <- dplyr::bind_rows(purrr::map(Ks, function(k) { 
		visObj <- largeVis(data, K = k, sgd_batches = 1)
		dplyr::bind_rows(purrr::map(Ms, function(m) {
			coords <- projectKNNs(visObj$wij, seed = 1974, M = m)
			new_data <- data.frame(t(coords))
			new_data <- set_colnames(new_data, c("x", "y"))
			mutate(new_data, label = labels, M = m, K = k)
		}))
	}))
...
As shown, increasing `K` encourages the algorithm to find relationships between groupings in the data. Increasing `M` encourages greater distinctiveness between groupings in the lower-dimensional space.
...
Adding momentum can make it possible to reduce the number of sgd batches, in some cases substantially, without reducing the visual quality of the result.  Setting $\lambda$ to between 0.4 and 0.9, and the number of batches to between 0.3 and 0.5, generally produces good results.


### The `manifoldMap` Function
...
```{r youtube,echo=T,fig.width=7,fig.height=14,warning=F,message=F}
if (.build_youtube) {
	library(Matrix)
	pathToGraphFile <- params$graph_file
	pathToCommunities <- params$communities
	youtube <-
		readr::read_tsv(pathToGraphFile, skip = 4, col_names = FALSE)
	youtube <- as.matrix(youtube)
	youtube <- Matrix::sparseMatrix(
		i = youtube[, 1],
		j = youtube[, 2],
	...
	for (i in 1:length(communities))
		community_assignments[communities[[i]]] <- i
	
	wij <- buildWijMatrix(youtube)
	wij_coordinates <- projectKNNs(wij, seed = 1974)
	
	# The input matrix for projectKNNs must be symmetric
	youtube <- youtube + t(youtube)
	direct_coordinates <- projectKNNs(youtube, seed = 1974)
	
	youTube_coordinates <-
		data.frame(rbind(t(wij_coordinates), t(direct_coordinates)))
	colnames(youTube_coordinates) <- c("x", "y")
	youTube_coordinates$community <-
		factor(rep(community_assignments, 2))
	youTube_coordinates$set <-
		rep(c("wij", "direct"), each = ncol(wij_coordinates))
	
	youTube_coordinates <- arrange(
		mutate(
			youTube_coordinates,
		...
		if_else(is.na(community), 0, 1)
	)
	
	ggplot(youTube_coordinates,
				 aes(
				 	x = x,
				 	y = y,
				 	color = community,
...
Memory requirements during the neighbor search may be managed by reducing `n_trees` or building the ANNOY index on-disk using the `save_file` parameter to `randomProjectionTreeSearch`. The decrease in precision from reducing the number of trees is marginal, and may be compensated-for by increasing `max_iters`. 
...
hdbscan_set <- expand.grid(minPts = c(6, 10, 20), K = c(2, 6, 12))

set <- dplyr::bind_rows(purrr::map2(hdbscan_set$minPts, hdbscan_set$K, function(x, y) {
...
```{r hdbscanobj}
hdclust <- largeVis::hdbscan(vis, K = 2, minPts = 10)
str(hdclust)
```
