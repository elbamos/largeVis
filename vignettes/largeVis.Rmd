...
youTube_coordinates2$community <- factor(community_assignments)
youTube_coordinates2$alpha <- factor(ifelse(youTube_coordinates2$community == 0, 0.05, 0.2))
```

```{r drawYouTube,echo=F}
load(system.file("extdata", "youtube.Rda", package="largeVis"))
ggplot(youTube_coordinates2, aes( x = x, 
                      y = y, 
                      color = community, 
                      alpha = alpha, 
                      size = alpha)) +
  geom_point() +
  scale_color_manual(values = c("black", colors_continuous(5000)), guide = FALSE) +
  scale_alpha_manual(values = c(0.01, 0.2), guide = FALSE) +
  scale_size_manual(values = c(0.05, 0.15), guide = FALSE) +
  scale_x_continuous("", 
