...
if (rebuild) {
	...
	timex <- system.time(firstCoords <- data.frame(t(
            projectKNNs(preVis$wij, coords = t(starterCoords), verbose = TRUE)), lambda = 0, batches = 1, label = labels))
...
momentumCoords %>%
	ggplot(aes(x = X1, y = X2, color = label)) + 
	geom_point(size = 0.05, alpha = 0.3) + 
	facet_grid(batches ~ lambda, scales = "free") +
...
timex %>%
	ggplot(aes(x = batches, y = elapsed / 60)) + 
	geom_point(size = 0.5, alpha = 0.7) +
	y_scale_continuous("Elapsed Time (minutes)") +
	x_scale_continuous("n. Batches / Default") +
