...
vignette: >
  %\VignetteIndexEntry{Momentum and useDegree}
...
```{r drawdegree,fig.width=5,fig.height=4,fig.align='center'}
...
Adding momentum can make it possible to reduce the number of sgd batches, in some cases substantially, while maintaining the visual quality of the result. 
...
if (rebuild) {
	...
	for (lambda in c(0.3, 0.6, 0.9)) {
			for (batches in c(0.1, 0.3, 0.8)) {
...
```{r drawmomentum,warning=FALSE,fig.width=6,fig.height=5,fig.align='center'}
